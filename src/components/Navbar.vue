<template>
  <div class="fixed top-6 left-0 right-0 z-50 flex justify-center px-4">
    ...
    <nav
      ...
      <div class="hidden md:flex items-center gap-2">
        <a v-for="link in navLinks" :key="link.name" @click.prevent="scrollTo(link.href)" href="#"
          class="px-4 py-2 rounded-full text-lg font-medium transition-all duration-300 relative group" :class="[
    ...
    <div v-if="isMenuOpen"
      class="absolute top-full mt-4 right-4 w-48 bg-white/50 dark:bg-slate-900 py-6 rounded-lg shadow-2xl border border-slate-700 dark:border-slate-800 flex flex-col gap-4 md:hidden">
      <a v-for="link in navLinks" :key="link.name" @click.prevent="scrollTo(link.href)" href="#"
        class="text-lg font-medium px-6 dark:text-slate-300 p-2 rounded" :class="{
          'text-slate-100 dark:text-white bg-blue-500 font-bold': activeSection === link.href.substring(1),
          'hover:bg-slate-200 text-slate-800 dark:hover:bg-slate-800': activeSection !== link.href.substring(1)
        }">
        {{ link.name }}
      </a>
    </div>
